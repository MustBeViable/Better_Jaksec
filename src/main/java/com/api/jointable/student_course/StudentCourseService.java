...
public class StudentCourseService {
    ...
    public StudentCourseDto create(CreateStudentCourse request){
        StudentCourse studentCourse = this.mapper.toEntity(request);
        studentCourse.setCourse(this.courseRepository.findById(request.getCourseId())
                .orElseThrow(()->new BadRequestException("Invalid course id")));
        studentCourse.setStudent(this.studentRepository.findById(request.getStudentId())
    ...
    public StudentCourseDto update(Long gradeId, UpdateStudentCourse request){
        ...
        if(request.getCourseId() != null) studentCourse.setCourse(this.courseRepository.findById(request.getCourseId()).get());
        if(request.getStudentId() != null) studentCourse.setStudent(this.studentRepository.findById(request.getStudentId()).get());
        return this.mapper.toDto(studentCourse);
    }
}
